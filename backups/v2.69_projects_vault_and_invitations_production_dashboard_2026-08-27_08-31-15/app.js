...
function setupEventTypeSwitcher() {
  ...
  const btnModeXv = document.getElementById('btnModeXv');
  const btnModeBoda = document.getElementById('btnModeBoda');

  if (btnModeXv && btnModeBoda) {
    btnModeXv.addEventListener('click', () => {
      btnModeXv.classList.add('active');
      btnModeBoda.classList.remove('active');
      selectType.value = 'xv';
      selectType.dispatchEvent(new Event('change'));
    });
    btnModeBoda.addEventListener('click', () => {
      btnModeBoda.classList.add('active');
      btnModeXv.classList.remove('active');
...
function setupDeviceSwitcher() {
  ...
  if (btnZoomOut) {
    btnZoomOut.addEventListener('click', () => {
      currentZoom = Math.max(0.6, Math.round((currentZoom - 0.1) * 10) / 10);
      applyZoom();
    });
  }
...
function setupThemePicker() {
  const cards = document.querySelectorAll('.theme-card, .theme-visual-card');
...
function populateForm() {
  ...
  const inWaxCustom = document.getElementById('inputWaxSealCustom');
  const btnClrWax = document.getElementById('btnClearWaxSeal');
  const waxCtrls = document.getElementById('waxSealControls');
  if (chkWax) {
    chkWax.checked = ws.enabled !== false;
    if (waxCtrls) waxCtrls.style.display = chkWax.checked ? 'flex' : 'none';
  }
  if (selWax) selWax.value = ws.preset || 'gold';
  if (inWaxPrompt) inWaxPrompt.value = ws.promptText || 'Toca el sello para abrir';
  if (inWaxCustom) inWaxCustom.value = ws.customImage || '';
  if (btnClrWax) btnClrWax.style.display = ws.customImage ? 'inline-flex' : 'none';
  ...
  // Estilo Editorial & Desplazamiento de Familia
  const famStyle = currentConfig.familyStyle || {};
  const famSize = famStyle.fontSize !== undefined ? famStyle.fontSize : 19;
  const pOffset = famStyle.parentsOffset !== undefined ? famStyle.parentsOffset : 0;
  const gpOffset = famStyle.godparentsOffset !== undefined ? famStyle.godparentsOffset : 0;
  const ctOffset = famStyle.courtOffset !== undefined ? famStyle.courtOffset : 0;

  const rngFamSize = document.getElementById('rangeFamilyFontSize');
  const valFamSize = document.getElementById('valFamilyFontSize');
  if (rngFamSize) rngFamSize.value = famSize;
  if (valFamSize) valFamSize.textContent = famSize + 'px';

  const rngParentsOffset = document.getElementById('rangeParentsOffset');
  const valParentsOffset = document.getElementById('valParentsOffset');
  if (rngParentsOffset) rngParentsOffset.value = pOffset;
  if (valParentsOffset) valParentsOffset.textContent = (pOffset > 0 ? '+' : '') + pOffset + 'px';

  const rngGodparentsOffset = document.getElementById('rangeGodparentsOffset');
  const valGodparentsOffset = document.getElementById('valGodparentsOffset');
  if (rngGodparentsOffset) rngGodparentsOffset.value = gpOffset;
  if (valGodparentsOffset) valGodparentsOffset.textContent = (gpOffset > 0 ? '+' : '') + gpOffset + 'px';

  const rngCourtOffset = document.getElementById('rangeCourtOffset');
  const valCourtOffset = document.getElementById('valCourtOffset');
  if (rngCourtOffset) rngCourtOffset.value = ctOffset;
  if (valCourtOffset) valCourtOffset.textContent = (ctOffset > 0 ? '+' : '') + ctOffset + 'px';
...
function setupFamilyStyleControls() {
  const ensureFamilyStyle = () => {
    if (!currentConfig.familyStyle) {
      currentConfig.familyStyle = { fontSize: 19, parentsOffset: 0, godparentsOffset: 0, courtOffset: 0 };
    }
  };

  const updateFamilyOffsets = (p, gp, ct) => {
    ensureFamilyStyle();
    currentConfig.familyStyle.parentsOffset = p;
    currentConfig.familyStyle.godparentsOffset = gp;
    currentConfig.familyStyle.courtOffset = ct;

    const rP = document.getElementById('rangeParentsOffset');
    const vP = document.getElementById('valParentsOffset');
    const rGP = document.getElementById('rangeGodparentsOffset');
    const vGP = document.getElementById('valGodparentsOffset');
    const rCT = document.getElementById('rangeCourtOffset');
    const vCT = document.getElementById('valCourtOffset');

    if (rP) rP.value = p;
    if (vP) vP.textContent = (p > 0 ? '+' : '') + p + 'px';
    if (rGP) rGP.value = gp;
    if (vGP) vGP.textContent = (gp > 0 ? '+' : '') + gp + 'px';
    if (rCT) rCT.value = ct;
    if (vCT) vCT.textContent = (ct > 0 ? '+' : '') + ct + 'px';

    schedulePreviewUpdate();
  };

  // Range font size
  const rngSize = document.getElementById('rangeFamilyFontSize');
  const valSize = document.getElementById('valFamilyFontSize');
  if (rngSize) {
    rngSize.addEventListener('input', (e) => {
      ensureFamilyStyle();
      const val = parseInt(e.target.value, 10) || 19;
      currentConfig.familyStyle.fontSize = val;
      if (valSize) valSize.textContent = val + 'px';
      schedulePreviewUpdate();
    });
  }

  // Range parents offset
  const rngParents = document.getElementById('rangeParentsOffset');
  const valParents = document.getElementById('valParentsOffset');
  if (rngParents) {
    rngParents.addEventListener('input', (e) => {
      ensureFamilyStyle();
      const val = parseInt(e.target.value, 10) || 0;
      currentConfig.familyStyle.parentsOffset = val;
      if (valParents) valParents.textContent = (val > 0 ? '+' : '') + val + 'px';
      schedulePreviewUpdate();
    });
  }

  // Range godparents offset
  const rngGodparents = document.getElementById('rangeGodparentsOffset');
  const valGodparents = document.getElementById('valGodparentsOffset');
  if (rngGodparents) {
    rngGodparents.addEventListener('input', (e) => {
      ensureFamilyStyle();
      const val = parseInt(e.target.value, 10) || 0;
      currentConfig.familyStyle.godparentsOffset = val;
      if (valGodparents) valGodparents.textContent = (val > 0 ? '+' : '') + val + 'px';
      schedulePreviewUpdate();
    });
  }

  // Range court offset
  const rngCourt = document.getElementById('rangeCourtOffset');
  const valCourt = document.getElementById('valCourtOffset');
  if (rngCourt) {
    rngCourt.addEventListener('input', (e) => {
      ensureFamilyStyle();
      const val = parseInt(e.target.value, 10) || 0;
      currentConfig.familyStyle.courtOffset = val;
      if (valCourt) valCourt.textContent = (val > 0 ? '+' : '') + val + 'px';
      schedulePreviewUpdate();
    });
  }

  // Presets
  const btnCenter = document.getElementById('btnPresetLayoutCenter');
  if (btnCenter) btnCenter.addEventListener('click', () => updateFamilyOffsets(0, 0, 0));

  const btnStagger = document.getElementById('btnPresetLayoutStagger');
  if (btnStagger) btnStagger.addEventListener('click', () => updateFamilyOffsets(-24, 24, 0));

  const btnLeft = document.getElementById('btnPresetLayoutLeft');
  if (btnLeft) btnLeft.addEventListener('click', () => updateFamilyOffsets(-30, -30, -30));

  const btnRight = document.getElementById('btnPresetLayoutRight');
  if (btnRight) btnRight.addEventListener('click', () => updateFamilyOffsets(30, 30, 30));
}
      ...
      reader.onload = (re) => {
        try {
          ...
          if (data.config) {
            ...
            document.querySelectorAll('.theme-card, .theme-visual-card').forEach(card => {
...
function setupWaxSealControls() {
  ...
  const fileInput = document.getElementById('fileWaxSeal');
  const customInput = document.getElementById('inputWaxSealCustom');
  const btnClear = document.getElementById('btnClearWaxSeal');
  const presetSelect = document.getElementById('selectWaxSealPreset');
  const promptInput = document.getElementById('inputWaxSealPrompt');

  if (!currentConfig.waxSeal) {
    currentConfig.waxSeal = {
      enabled: true,
      customImage: '',
      preset: 'gold',
  ...
  const updateControlsVisibility = () => {
    if (controlsDiv && checkEnabled) {
      controlsDiv.style.display = checkEnabled.checked ? 'flex' : 'none';
    }
  };

  if (checkEnabled) {
    checkEnabled.checked = currentConfig.waxSeal.enabled !== false;
    updateControlsVisibility();
    checkEnabled.addEventListener('change', (e) => {
      currentConfig.waxSeal.enabled = e.target.checked;
      updateControlsVisibility();
      schedulePreviewUpdate();
    });
  }
  ...
  if (presetSelect) {
    presetSelect.value = currentConfig.waxSeal.preset || 'gold';
    presetSelect.addEventListener('change', (e) => {
      currentConfig.waxSeal.preset = e.target.value;
      schedulePreviewUpdate();
    });
  }

  if (promptInput) {
    promptInput.value = currentConfig.waxSeal.promptText || 'Toca el sello para abrir';
    promptInput.addEventListener('input', (e) => {
      currentConfig.waxSeal.promptText = e.target.value;
      schedulePreviewUpdate();
    });
  }
  ...
  if (fileInput && customInput) {
    fileInput.addEventListener('change', (e) => {
      ...
      if (file) {
        const reader = new FileReader();
        reader.onload = (re) => {
          customInput.value = re.target.result;
          currentConfig.waxSeal.customImage = re.target.result;
          if (btnClear) btnClear.style.display = 'inline-flex';
          schedulePreviewUpdate();
          showToast('Sello personalizado cargado');
        };
        reader.readAsDataURL(file);
      }
    });
    ...
    customInput.addEventListener('input', (e) => {
      currentConfig.waxSeal.customImage = e.target.value;
      if (btnClear) btnClear.style.display = e.target.value ? 'inline-flex' : 'none';
      schedulePreviewUpdate();
    });
  }
  ...
  if (btnClear) {
    btnClear.addEventListener('click', () => {
      ...
      currentConfig.waxSeal.customImage = '';
      btnClear.style.display = 'none';
      schedulePreviewUpdate();
      showToast('Sello personalizado eliminado (usando predeterminado)');
    });
  }
}
