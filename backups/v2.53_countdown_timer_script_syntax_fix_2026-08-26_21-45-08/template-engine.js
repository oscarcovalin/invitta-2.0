...
if(CONFIG.eventDateISO){
  let rawDate = String(CONFIG.eventDateISO).trim();
  if (rawDate.includes(' ') && !rawDate.includes('T')) {
    rawDate = rawDate.replace(' ', 'T');
  }
  const tz = String(CONFIG.timezoneOffset || '').trim();
  let target = NaN;
  if (tz && !rawDate.endsWith('Z') && !/[+-]\d{2}:\d{2}$/.test(rawDate)) {
    target = new Date(rawDate + tz).getTime();
  }
  if (isNaN(target)) {
