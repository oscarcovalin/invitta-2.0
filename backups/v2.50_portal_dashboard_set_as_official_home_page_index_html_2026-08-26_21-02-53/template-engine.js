    ...
    if (contenedorSaludo) {
      contenedorSaludo.innerHTML = '<div class="mensaje-personalizado shadow-sm border border-antique-gold/40 rounded-2xl bg-amber-500/10 p-3.5 mb-3 text-left">' +
        '<div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-antique-gold/20 border border-antique-gold/50 text-deep-onyx font-bold text-xs mb-2 font-mono">' +
          '<span>🪑</span> <span>' + mesaLabelTxt.toUpperCase() + ' · ' + ticketsParam + (ticketsParam === 1 ? ' PASE ASIGNADO' : ' PASES ASIGNADOS') + '</span>' +
        '</div>' +
        '<h3 class="font-display-lg text-lg text-deep-onyx font-bold mb-0.5">' + nombreDecodificado + '</h3>' +
...
(function initSecureCollaborativeAlbum() {
  ...
  // 4. Subida y Almacenamiento en DB
  function savePhotosToStorage(photosData) {
    try {
      const stored = JSON.parse(localStorage.getItem('invitta_album_photos_db') || '[]');
      const updated = stored.concat(photosData);
      localStorage.setItem('invitta_album_photos_db', JSON.stringify(updated));
    } catch(e) {
  ...
  if (btnSubmit) {
    btnSubmit.addEventListener('click', () => {
      ...
      btnSubmit.disabled = true;
      btnSubmit.textContent = 'Guardando fotos...';
    ...
    recent.forEach(p => {
      html += 
        '<div class="group relative rounded-xl overflow-hidden aspect-square border border-antique-gold/50 shadow-sm bg-neutral-900 flex flex-col">' +
          '<img src="' + p.photoData + '" alt="Foto de ' + (p.guestName || 'Invitado') + '" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>' +
          '<div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent p-1 sm:p-1.5 text-left">' +
            '<p class="text-[9px] sm:text-[10px] text-amber-200 font-bold truncate leading-tight">' + (p.guestName || 'Invitado') + '</p>' +
            '<p class="text-[8px] text-neutral-300 font-mono">' + (p.time || '') + '</p>' +
          </div>' +
        '</div>';
    });
  ...
  function renderBrideGallery() {
    ...
    photosGrid.innerHTML = photos.map(p => {
      const dedicationHtml = p.dedication ? ('<p class="text-[10px] text-neutral-300 italic line-clamp-2 mt-0.5">&ldquo;' + p.dedication + '&rdquo;</p>') : '';
      return '<div class="bg-neutral-900 border border-antique-gold/30 rounded-2xl overflow-hidden flex flex-col justify-between shadow-md">' +
        '<div class="relative aspect-square w-full bg-black overflow-hidden group">' +
          ...
          '<a href="' + p.photoData + '" download="recuerdo_' + p.id + '.jpg" class="absolute bottom-2 right-2 w-7 h-7 rounded-full bg-black/80 text-white flex items-center justify-center text-xs hover:bg-antique-gold hover:text-black transition-colors" title="Descargar foto">💾</a>' +
        '</div>' +
