...
(function initRsvpPergamino() {
  ...
  if (guestParam && guestParam.trim()) {
    ...
    if (urlMesaParam) {
      mesaLabelTxt = urlMesaParam.toLowerCase().includes('imp') ? 'Mesa Imperial' : ('Mesa ' + urlMesaParam.replace(/\D/g, ''));
    }
      ...
      return '<div class="bg-neutral-900 border border-antique-gold/30 rounded-2xl overflow-hidden flex flex-col justify-between shadow-md">' +
        ...
        '<div class="p-2.5 text-left bg-neutral-900/90">' +
          '<div class="flex items-center justify-between gap-1 mb-0.5">' +
            '<strong class="text-[11px] text-amber-200 truncate">' + (p.guestName || 'Invitado') + '</strong>' +
            '<span class="text-[9px] text-neutral-400 font-mono">' + (p.time || '') + '</span>' +
          </div>' +
          dedicationHtml +
        '</div>' +
      '</div>';
