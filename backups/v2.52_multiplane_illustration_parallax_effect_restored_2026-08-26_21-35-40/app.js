...
function populateForm() {
  ...
  const countText = countStyle.textColor || '#ffffff';
    ...
    fileAudio.addEventListener('change', (e) => {
      ...
      if (file) {
        ...
        reader.onload = (re) => {
          ...
          if (!currentConfig.music.title || currentConfig.music.title === 'Música') {
            currentConfig.music.title = file.name.replace(/\.[^/.]+$/, "");
          }
          if (inputMusicUrl) inputMusicUrl.value = re.target.result;
          const inTitle = document.getElementById('inputMusicTitle');
          if (inTitle && (!inTitle.value || inTitle.value === 'Música')) inTitle.value = currentConfig.music.title;
          schedulePreviewUpdate();
...
// Presets de Ilustraciones Botánicas y Florales de Alta Definición
const ILLUSTRATION_PRESETS = {
  goldArch: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 280" fill="none"><defs><linearGradient id="goldG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="%23E5C07B"/><stop offset="50%" stop-color="%23D4AF37"/><stop offset="100%" stop-color="%23AA7C11"/></linearGradient><filter id="glow"><feGaussianBlur stdDeviation="3" result="coloredBlur"/><feMerge><feMergeNode in="coloredBlur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><g filter="url(%23glow)"><path d="M50 200 C 200 40, 600 40, 750 200" stroke="url(%23goldG)" stroke-width="2.5" fill="none"/><path d="M120 180 C 250 80, 550 80, 680 180" stroke="url(%23goldG)" stroke-width="1.5" stroke-dasharray="6 4" fill="none"/><circle cx="400" cy="70" r="14" fill="none" stroke="url(%23goldG)" stroke-width="2"/><circle cx="400" cy="70" r="6" fill="url(%23goldG)"/><path d="M380 70 Q 400 45 420 70 Q 400 95 380 70 Z" fill="url(%23goldG)" opacity="0.75"/><g fill="url(%23goldG)"><circle cx="280" cy="100" r="5"/><path d="M270 95 Q 285 75 300 95 Q 285 115 270 95 Z" opacity="0.8"/><circle cx="520" cy="100" r="5"/><path d="M510 95 Q 525 75 540 95 Q 525 115 510 95 Z" opacity="0.8"/><circle cx="200" cy="140" r="4"/><path d="M190 135 Q 205 115 220 135 Q 205 155 190 135 Z" opacity="0.7"/><circle cx="600" cy="140" r="4"/><path d="M590 135 Q 605 115 620 135 Q 605 155 590 135 Z" opacity="0.7"/></g></g></svg>',
  oliveBranch: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 260" fill="none"><defs><linearGradient id="sageG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="%238DA399"/><stop offset="50%" stop-color="%236B8E7B"/><stop offset="100%" stop-color="%23C2A649"/></linearGradient></defs><g stroke="url(%23sageG)" fill="none"><path d="M80 180 Q 400 60 720 180" stroke-width="2.5"/><g fill="url(%23sageG)" opacity="0.85"><path d="M220 135 C 210 105 240 100 250 130 C 250 145 230 150 220 135 Z"/><path d="M300 105 C 290 75 320 70 330 100 C 330 115 310 120 300 105 Z"/><path d="M380 90 C 375 60 405 55 410 85 C 410 100 390 105 380 90 Z"/><path d="M480 100 C 490 70 460 65 450 95 C 450 110 470 115 480 100 Z"/><path d="M560 120 C 570 90 540 85 530 115 C 530 130 550 135 560 120 Z"/><path d="M640 150 C 650 120 620 115 610 145 C 610 160 630 165 640 150 Z"/></g><g fill="%234A5D4E" opacity="0.9"><circle cx="260" cy="140" r="6"/><circle cx="350" cy="105" r="6"/><circle cx="440" cy="100" r="6"/><circle cx="530" cy="125" r="6"/></g></g></svg>',
  champagneFlowers: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 260" fill="none"><defs><linearGradient id="roseG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="%23F4D0C5"/><stop offset="50%" stop-color="%23E0A899"/><stop offset="100%" stop-color="%23C59B27"/></linearGradient></defs><g stroke="url(%23roseG)"><path d="M100 190 Q 400 40 700 190" stroke-width="2" fill="none"/><g fill="url(%23roseG)"><circle cx="400" cy="80" r="22" opacity="0.3"/><circle cx="400" cy="80" r="14" opacity="0.6"/><circle cx="400" cy="80" r="6"/><circle cx="310" cy="115" r="16" opacity="0.3"/><circle cx="310" cy="115" r="10" opacity="0.6"/><circle cx="310" cy="115" r="4"/><circle cx="490" cy="115" r="16" opacity="0.3"/><circle cx="490" cy="115" r="10" opacity="0.6"/><circle cx="490" cy="115" r="4"/><path d="M380 60 Q 400 30 420 60 Z" opacity="0.75"/><path d="M350 95 Q 330 75 355 70 Z" opacity="0.75"/><path d="M450 95 Q 470 75 445 70 Z" opacity="0.75"/></g></g></svg>',
  botanicalCrest: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 260" fill="none"><defs><linearGradient id="crestG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="%23F7F6EC"/><stop offset="50%" stop-color="%23D4AF37"/><stop offset="100%" stop-color="%23996515"/></linearGradient></defs><g stroke="url(%23crestG)" fill="none"><path d="M260 170 C 220 90, 360 40, 400 80 C 440 40, 580 90, 540 170" stroke-width="2.5"/><polygon points="400,60 412,80 400,100 388,80" fill="url(%23crestG)" opacity="0.85"/><circle cx="400" cy="80" r="3" fill="%23fff"/><g fill="url(%23crestG)" opacity="0.7"><path d="M300 130 C 290 110, 315 100, 320 120 Z"/><path d="M340 100 C 330 80, 355 70, 360 90 Z"/><path d="M500 130 C 510 110, 485 100, 480 120 Z"/><path d="M460 100 C 470 80, 445 70, 440 90 Z"/></g></g></svg>'
};
...
function setupFileUploads() {
  const map = [
    { fileId: 'filePhotoHero', inputId: 'inputPhotoHero', path: 'photos.hero' },
    { fileId: 'filePhotoDate', inputId: 'inputPhotoDate', path: 'photos.saveTheDate' },
    { fileId: 'filePhotoPortrait', inputId: 'inputPhotoPortrait', path: 'photos.portrait' }
  ];

  map.forEach(({ fileId, inputId, path }) => {
    const fileEl = document.getElementById(fileId);
